...
int matrixSumResult[MAX][MAX]; 
int matrixDiffResult[MAX][MAX]; 
int matrixProductResult[MAX][MAX]; 
...
void* computeSum(void* args) {
	...
	matrixSumResult[row][col] = matA[row][col] + matB[row][col];
}
...
void* computeDiff(void* args) {
   int *index = (int*) args;
	...
	matrixDiffResult[row][col] = matA[row][col] - matB[row][col];
}
...
void* computeProduct(void* args) {
	...
	matrixProductResult[row][col] += matA[row][col] * matB[row][col];
}
...
int main() {
   ...
   fillMatrix(matA);
    ...
    printf("Results:\n");
    printf("Sum:\n");
    printMatrix(matrixSumResult);
    printf("Difference:\n");
    printMatrix(matrixDiffResult);
    printf("Product:\n");
    printMatrix(matrixProductResult);
    return 0;
}